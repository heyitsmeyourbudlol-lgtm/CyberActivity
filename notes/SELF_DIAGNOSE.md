...
_Updated 2026-09-08 00:27:29_ · self-sufficient · errors + miscalculations + poor logic
...
**Error:**
- **[high] Queue drift mislabeled as ADAPT_STALE** (error) — playbook:adapt_stale_mislabel_queue → `./scripts/peer sync-queue` · `./scripts/peer adapt`
  - Logic: Queue errors are Institutional memory — sync-queue first. adapt_state only for fingerprint.
- **[high] WORK_QUEUE ↔ self_improve_context mismatch** (error) — playbook:queue_drift → `./scripts/peer sync-queue` · `./scripts/peer queue-status`
  - Logic: Make open items identical in both files; never edit only one.
- **[medium] WORK_QUEUE ↔ context drift** (error) — only in self_improve_context: **[top10] Newdrop production — next meaningful non-UI merge** — after #104; file; only in  → `./scripts/peer sync-queue` · `./scripts/peer queue-status`
  - Logic: Make open items identical in both files; never edit only one.

**Miscalculation:**
- **[high] WORK_QUEUE ↔ self_improve_context drift** (miscalculation) — only in notes/WORK_QUEUE.md: **[top10] Newdrop production — next meaningful non-UI merge** — after #104; file → `./scripts/peer heal-all`
  - Logic: Queue Steward: sync pair to identical open items.
