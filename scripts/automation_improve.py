...
def queue_known_keys(*, work_md: str | None = None, context_md: str | None = None) -> set[str]:
    ...
    # Prefer single-pass index (QUEUE_MD_INDEX_SINGLE_PASS) when available —
    # shares memo with open_done_orphan / sync_queue_drift in the same gather.
    keys: set[str] = set()
    for md in (work_md, context_md):
        if not md:
            continue
        if hasattr(auto, "queue_md_known_keys"):
