...
def release_anon_pressure(*, log_fn: Callable[[str], None] = print, keep_chunks: int = 4) -> int:
    ...
    # Under-target: only peel on true OOM. CUDA unified spikes briefly drive
    # MemAvailable to 2–4GB; peeling at 2.0 undid 86→50 climbs every burst.
    hard_oom = 1.0 if under_target else crit
...
def free_headroom_for_fill(*, log_fn: Callable[[str], None] = print) -> dict[str, int]:
    ...
    anon_freed = 0
    # Match release_anon under-target hard_oom=1.0 — do not dump climb on
    # brief CUDA unified MemAvailable dips to 2–4GB.
    if stats["avail_gb"] < (1.0 if gap > hold_band_gb() else 2.0):
