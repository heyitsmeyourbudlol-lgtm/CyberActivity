    ...
    if mode == "hold":
        ...
        if priority.enabled() and avail_critical:
            ...
            if priority._fill_pressure_blocked():
                ...
                # Leave modest headroom for GPU teacher — old 12–14GB floor froze
                # climb at ~50–80GB while CUDA reserved 30–45GB (unified GB10).
                gpu_floor = max(floor, 5.5)
                try:
    ...
    while True:
        try:
            ...
            if isinstance(pri, dict):
                ...
                # Swap-dead + large CUDA reserved: 0.25GB only — 0.5×N OOM-killed
                # holder at ~80GB peak (needle 2026-09-08).
                avail_tick = float(budget.mem_stats()["avail_gb"])
                swap_free = float(priority._swap_free_gb())
                gpu_live = False
                try:
                    import subprocess as _sp
                    ...
                    gpu_live = False
                if swap_free < 0.5 or (gpu_live and avail_tick < 12.0):
                    pri["fill_chunk_gb"] = 0.25
                else:
                    pri["fill_chunk_gb"] = 0.25 if avail_tick < 20 else 0.5
                pri["min_fill_avail_gb"] = min(
                    max(float(pri.get("min_fill_avail_gb") or 4.5), 4.5), 5.5
                )
                # When teacher is live + swap dead, keep a higher floor so anon
                # climb does not OOM-kill this holder (fp collapse → GPU thrash).
                if gpu_live and swap_free < 0.5:
                    pri["min_fill_avail_gb"] = max(float(pri["min_fill_avail_gb"]), 6.0)
