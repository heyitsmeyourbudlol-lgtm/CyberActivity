...
def _improve_log_path() -> Path:
    ...
    # IMPROVE_LOG_PATH_TARGETED_SCANDIR_2026_09_08 — no full ~/.config glob.
    try:
        with os.scandir(home_cfg) as it:
            for ent in it:
                try:
                    ...
                    continue
                name = ent.name
                if name in _IMPROVE_LOG_FALLBACK_NS or name.startswith(
                    ("peer-", "automation")
                ):
                    cand = Path(ent.path) / "improve-loop.log"
                    if cand not in candidates:
                        candidates.append(cand)
    except OSError:
        pass
    ...
    for cand in candidates:
        ...
        if mtime > best_mtime:
            best_mtime = mtime
            best = cand
    chosen = best if best is not None else IMPROVE_LOG
